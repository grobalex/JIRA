...
def createJSONPayload(JIRA_GROUP, permissions, permissionschemename): 
	...
	for permission in permissions:
		for group in JIRA_GROUP:
			temp.append('{"holder": {"type": "group","parameter":"' + group + '"},"permission": "' + permission + '"}')
...
def APIrequest(url, typeOfRequest, data):
	if typeOfRequest == 'post':
		response=requests.post(url , verify=False , auth=(username, pswd), headers={'Accept': 'application/json', 'Content-Type': 'application/json'}, json = data)
	elif typeOfRequest == 'get':
		#GET /rest/api/2/project/{projectIdOrKey}
		print('get')	
	elif typeOfRequest == 'put':
		response=requests.put(url , verify=False , auth=(username, pswd), headers={'Accept': 'application/json', 'Content-Type': 'application/json'}, json = data)
	elif typeOfRequest == 'delete':
		response=requests.delete(url , verify=False , auth=(username, pswd), headers={'Accept': 'application/json'}) 
	elif typeOfRequest == 'post':
		response=requests.post(url , verify=False , auth=(username, pswd), headers={'Accept': 'application/json'}) 
	else:
		raise ValueError('Type of Request {} is not valid'.format(typeOfRequest))
	try:
...
def createCompleteProject(name, key,projectTypeKey,projectTemplateKey,description,lead, users, ldap):
	...
	if ldap is not None and users is None:
		permissionSchemeKey = APIrequest(baseapiurl + 'permissionscheme', 'post', json.loads(createJSONPayload([ldap], jirapermissions, name+'-PS')))
	else:
		permissionSchemeKey = APIrequest(baseapiurl + 'permissionscheme', 'post', json.loads(createJSONPayload([name+'-group'], jirapermissions, name+'-PS')))
		#print(permissionSchemeKey)
		addUsersToGroup(users, baseapiurl + 'group/user?groupname='+name+'-group')
	createProject(key, name, projectTypeKey, projectTemplateKey, description, lead, permissionSchemeKey)
...
def userInput():
	...
	while True:
		...
		if special_match(key):
			...
			continue	
	projectTypeKey = input('Should this be a Software (s) or Business (b) project?')
	print(projectTypeKey)
	if projectTypeKey.lower() == 's':
		projectTypeKey = 'software'
		while True:
			projectTemplate = input('What template should be used? Scrum (s) , Kanban (k) or Soft. Dev. (sd): ')
		...
		while True:
			projectTemplate = input('What template should be used? Project (p) , Task (t) or Process (ps) management ')
			projectTemplate = projectTemplate.lower()
	...
	if uses_ldap.lower() == 'y':
		...
		users = input('Input a list of users seperated by a comma: ').split(",")
		ldap = None	
	createCompleteProject(name, key,projectTypeKey,projectTemplateKey,description,lead, users, ldap)
...
def fileHandler(PATH):
	...
	if wb2.sheetnames[0] == 'groups' and wb2.sheetnames[1] == 'persons':
		for project in iterateOverRows(wb2[wb2.sheetnames[0]]):
			user_list = getUser(iterateOverRows(wb2[wb2.sheetnames[1]]),project[1])
			createCompleteProject(project[0], project[1],project[2],project[3],project[4],user_list[0], user_list, None)
...
def main():
	if len(sys.argv) > 1:
		if sys.argv[1] == '--input':
			...
			if len(sys.argv) == 4:
				getLoginCreds()
				addUsersToGroup(sys.argv[3].split(","), baseapiurl + 'group/user?groupname='+sys.argv[2])
			else:
...
#createCompleteProject('test3', 'T3', 'software','com.pyxis.greenhopper.jira:gh-scrum-template', 'test', 'agrob',['agrob','maw'])
...
#APIrequest(baseapiurl + 'group/user?groupname=example-group', 'post', {'name' : 'agrob'})
...
#APIrequest(baseapiurl + 'group/user?groupname=example-group', 'delete', {'name' : 'agrob'})
